...
client = commands.AutoShardedBot(command_prefix = commands.when_mentioned_or("b!","B!"),help_command=None,shard_count=2)
...
async def on_ready():
  ...
  await client.change_presence(activity=discord.Activity(type=discord.ActivityType.watching,name=f' {members} members in {servers} servers.'))


async def ch_pr():
 await client.wait_until_ready()

 statuses = ["b!help and b!guide","Thanks for 80+ servers! | b!help","Family Guy :D | b!help"]
...
async def on_command_error(ctx, error):
    if isinstance(error, commands.CommandNotFound):
      e=discord.Embed(title="Internal Error!!",value="The command you entered was not a valid command, check b!help for a list of commands!")
      e.add_field(name="Error Value :",value=f"```python3\n{error}```")
      await ctx.send(embed=e)
    elif isinstance(error, commands.CommandOnCooldown):
     await ctx.send(f'Slow down this command is in cooldown for `{error.retry_after:.2f}`seconds.')
...
async def on_message(message):
  ...
  if message.content == mention:
    await message.channel.send('My prefix is **b!** for help run the command `b!help` and use ``b!guide`` if you need or you can ask for help in the support server, For an invite run the command `b!server`')
...
async def reload(ctx, extension=None):
    ...
    embed = discord.Embed(title='Reloaded :white_check_mark:', description=f'{extension}.py successfully reloaded', color=0xff00c8)
    await ctx.send(embed=embed)
...
async def botinfo(ctx):
    ...
    embed.add_field(name="<:servers:898170039595831306> Servers ",value=f"{len (client.guilds)}")
...
async def feedback(ctx,* ,suggestion=None):
  if ctx.author.id in client.blacklisted:
    user = discord.Member
    await ctx.send(f'{user.mention}, sorry but it looks like you have been blacklisted! (do b!server to join our server and ask admins there!)')
    return
  if suggestion == None:
    await ctx.send('Hey! there slowdown there bud, you have to give me a suggestion and not just a blank text :D')
    return
  else:
    ...
    fck.set_footer(text=f"Feedback by : {ctx.author.name}#{ctx.author.discriminator}")
...
async def blacklist(ctx,user:discord.Member):
  if ctx.author.id in manager:
    ...
    e=discord.Embed(title=f"<:blacklist:898493775914098688> {user} Has been blacklisted",color=0xe67e22)
    await ctx.send(embed=e)
  if ctx.author.id == 697323031919591454:
    ...
    e=discord.Embed(title="Sorry you dont have the permissions ```developer``` to use this command!",color=0xe67e22)
    await ctx.send(embed=e)
...
async def balance(ctx,* ,member=None):
    if member == None:
      ...
      em.add_field(name=f"Wallet Balance <a:money:891257220937945108>", value=wallet_amt,inline=False)
      em.add_field(name="Bank <:barbank:899206420547907625>", value=bank_amt,inline=False)
      if ctx.author.id == 697323031919591454:
       em.add_field(name="Badges | Profile Badges",value="<:developer:891263247452372993> <:staff:891264731212550174> <:owner:891265219664437309>")
      if ctx.author.id in helper:
        em.add_field(name="Badges | Profile Badges",value="<:Helper:898168928562475038>")
      if ctx.author.id in manager:
        em.add_field(name="Badges | Profile Badges",value="<:manager:899209735914536961>")
      em.set_thumbnail(url="https://images.discordapp.net/avatars/859713560774574101/a9c0ec8517ff31e32a8132678ebe3fbe.png?size=128")
      await ctx.send(embed=em)
    else:
      ...
      em = discord.Embed(title=f"{member.mention}'s balance.", color=discord.Color.red())
      em.add_field(name=f"Wallet Balance <a:money:891257220937945108>", value=wallet_amt,inline=False)
      em.add_field(name="Beer Storage <a:dbeer:891261524478402580>", value=bank_amt,inline=False)
      em.set_thumbnail(url="https://images.discordapp.net/avatars/859713560774574101/a9c0ec8517ff31e32a8132678ebe3fbe.png?size=128")
      await ctx.send(embed=em)
...
async def reset_cooldown_all(ctx):
  if ctx.author.id in owner:
    ...
    await ctx.send(f'{ctx.author.mention}, I have reset the cooldown of **all** commands that are set with a cooldown.')
  else:
